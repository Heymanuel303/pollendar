...
## Acceptance criteria
- [x] The winner reads via border/ring + colour, not a gold halo; no `0 0 28px` / `0 0 8px` glow values remain in `frontend/src`.
- [x] Zero user-facing "In bloom" / "blooms" / "like pollen" copy in `frontend/src`; the winning-slot pill reads "✦ Top pick".
- [x] No em dash (—) inside running prose in the Vue UI; the email optional label reads "Email (optional)".
- [x] The poll-completed email shows the "Top pick" chip and plain voice; the magic-link body has no em dash.
- [x] `frontend`: lint + type-check + test:unit, and `backend`: lint + test, all pass.
