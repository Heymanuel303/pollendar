...
## Acceptance criteria
- [ ] `npm test` (unit) and `npm run test:e2e` (e2e) both green against a disposable test schema.
- [ ] Validation rejects malformed bodies (whitelist / forbidNonWhitelisted) and unhandled Prisma errors return clean HTTP codes with no internal leak.
- [ ] Public submission endpoint is rate-limited; auth endpoints remain throttled.
- [ ] Security checklist satisfied: httpOnly + Secure cookies in prod, hashed tokens, anti-enumeration (200 on unknown email), no email leak, credentialed CORS allow-list.
- [ ] `docs/DEPLOY.md` documents image build, run order, `prisma migrate deploy`, and required prod env.
